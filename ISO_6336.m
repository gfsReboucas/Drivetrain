classdef ISO_6336 < Gear_Set
    ...
    methods
        ...
        function [N, sigma] = Pitting_SN_curve(obj)
            N = logspace(4, 10, 100);
            sigma = zeros(2, length(N));
            
            for idx = 1:length(N)
                sigma(:, idx) = obj.sigma_HP_ref.*obj.life_factor(N(idx));
            end
        end
    ...
    methods(Static)
        function calc = example_01()
            ...
            tab_set = {'Pitting safety factor for pinion'         , 'S_H1'         , calc.S_H(1)       , 1.02853;
...
%             default = {};
    ...
    methods(Access = private)
            ...
            elseif(strcmpi(obj.configuration, 'planetary'))
                ...
                switch(idx_zero)
                    case 1 % fixed: sun
                        ...
                        U(:, 4) = [0.0, rp*(1.0 + sr), (1.0 + sr), 1.0]';
                    case 2
                        error('ISO_6336:speed', 'Direct drive not implemented yet.');
                    case 3 % fixed: ring
                           % input: sun*
                           % output: carrier*
                        U(:, 4) = [(1.0 + rs), rp*(1.0 + rs), 0.0, 1.0]';
        ...
        function ZN = life_factor(obj, N)
            ...
            for idx = 1:2
                ...
                ratio = obj.sigma_HP_stat(idx)/obj.sigma_HP_ref(idx);
    ...
    methods
        ...
        function val = get.N_L(obj)
            ...
            N_L1 = obj.n_output*60.0*obj.L_h; % pinion
        ...
        function val = get.v_pitch_line(obj)
            val = nan;
            if(strcmp(obj.configuration, 'parallel'))
                val = (pi*obj.n_output/60.0e3)*obj.d(1);
            elseif(strcmp(obj.configuration, 'planetary'))
